...
export class MainPage {
    ...
    render() {
        ...
        this.addListeners();
        this.getData(); // загружаем все фильтры
    }

    getData(query = '') {
        ajax.get(filterUrls.getFilters(query), (data, status) => {
            if (status === 200 && data) {
                this.renderCards(data);
            } else {
                console.error('Ошибка загрузки фильтров', status);
    ...
    handleDelete(e) {
        ...
        ajax.delete(filterUrls.deleteFilter(id), (data, status) => {
